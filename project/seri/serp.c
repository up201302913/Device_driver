...
irqreturn_t short_interrupt(int irq, void *dev_id){
  struct serp_devs *dev = (struct serp_devs *) dev_id;
  char buffer;

  int IIR_ADDR = inb(ADR_COM1+UART_IIR);
  //HANDLE READ
  if((IIR_ADDR & UART_IIR_RDI)){

    kfifo_get(dev->rxbuf,&buffer,1);
...
/*
  ...
  request_irq(IRQ_NUM, irqreturn_t (*handler)(int, void *),
                unsigned long flags, const char *dev_name,NULL);
